...
{
    public partial class FaseA : Form
    {
        ...
        public FaseA()
        {
            ...
            x_axis_corrente = new DateTimeAxis(TimeSpan.FromSeconds(10), date => date.ToString("HH:mm:ss"))
            {
                MinLimit = null,
                MaxLimit = null,
            ...
            x_axis_tensao = new DateTimeAxis(TimeSpan.FromSeconds(10), date => date.ToString("HH:mm:ss"))
            {
                MinLimit = null,
                MaxLimit = null,
                ForceStepToMin = false,
        ...
        private void FaseA_Load(object sender, EventArgs e)
        {
        ...
        {
            Task.Run(() => update_chart_corrente());
            Task.Run(() => update_chart_tensao());
        }

        private void update_chart_corrente()
        {
            lock (M1DataHandler.lock_ca)
            {
                foreach (var (id_MU, buffer) in M1DataHandler.corrente_faseA)
                {
                    ...
                    //{
                        if (buffer.Count > 0)
                        {
                            ...
                            {
                                var ts = M1DataHandler.timestampA;
                                var i = buffer.Last();
        ...
        private void update_chart_tensao()
        {
            lock (M1DataHandler.lock_ta)
            {
                foreach (var (id_MU, buffer) in M1DataHandler.tensao_faseA)
                {
